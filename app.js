const defaultData = {
        ...
        projects: [
          {
            ...
            date: "2022.11 — 至今",
            summary:
              "面向年轻人的生活记录与目标管理工具｜React / TypeScript / Figma",
            logo: "",
            tone: "gray",
            projects: [
              {
        ...
        honors: [
          { title: "全国大学生广告艺术大赛", detail: "省级一等奖 · 2023" },
        ],
        skills: [
          {
            title: "设计工具",
        ...
        html += sectionEditor(
          ...
          (item) =>
            `<div class="field-grid">${input(
              ...
              "major",
              item.major
            )}${input("学校标签", "tier", item.tier)}${input(
              "补充信息",
              "detail",
              item.detail
        ...
        html += sectionEditor(
          ...
          (item) =>
            `<div class="field-grid">${input(
              ...
              "title",
              item.title
            )}${input("时间 / 级别", "detail", item.detail)}</div>`,
          "honor"
        );
        html += sectionEditor(
          "专业技能",
          ...
          (item) =>
            `<div class="field-grid">${input(
              ...
              "title",
              item.title
            )}${input("描述", "detail", item.detail, "textarea")}</div>`,
          "skill"
        );
      ...
      function companyEditor(item) {
        return `<div class="field-grid">${input(
          ...
          "role",
          item.role
        )}${input("视觉主题", "tone", item.tone)}${input(
          "概述与技术栈",
          "summary",
          item.summary,
          "textarea"
            ...
            .map(
              (project, index) =>
                `<div class="item-card" style="margin-top:7px"><div class="item-head"><strong>项目 ${
                  ...
                  `projects.${index}.role`,
                  project.role
                )}<div class="field full"><label>项目要点（每行一条）</label><textarea data-path="projects.${index}.bullets">${esc(
      ...
      function bindEditor() {
        ...
        $("#editorRoot")
          ...
          .forEach(
            (el) =>
              (el.onclick = () => {
                const [key, index, project] =
                  el.dataset.removeProject.split(".");
      ...
      function renderPreview() {
        ...
        let html = `<header class="resume-header${
          ...
          p.phone
        )}</span><span>✉ ${esc(p.email)}</span><span>⌖ ${esc(
          p.location
        )}</span>${
          p.website ? `<span class="resume-link">${esc(p.website)}</span>` : ""
        ...
        if (data.internships)
          html += `<section class="resume-section"><h2 class="resume-section-title">实习经历</h2>${data.internships
            ...
            .join("")}</section>`;
        if (data.projects)
          html += `<section class="resume-section"><h2 class="resume-section-title">项目经历</h2>${data.projects
            .map(employerHtml)
            .join("")}</section>`;
        if (data.honors)
        ...
        if (data.skills)
          html += `<section class="resume-section"><h2 class="resume-section-title">专业技能</h2><ul class="skills">${data.skills
            .map(
              (x) =>
                `<li><strong>${esc(x.title)}：</strong>${esc(x.detail)}</li>`
            )
            .join("")}</ul></section>`;
      ...
      function employerHtml(x) {
        return `<article class="employer"><div class="employer-band ${
          ...
          x.logo ? "" : ' style="grid-column:1/3"'
        }>${esc(x.company)}</div><div class="employer-date">${esc(
          x.date
      ...
      function projectHtml(x) {
        return `<section class="project"><div class="project-title-row"><h3 class="project-title">${esc(
          x.title
        )}</h3><span class="project-role">${esc(
          x.role
        )}</span></div><ul class="narrative">${(x.bullets || [])
          .map((b) => `<li>${formatBullet(b)}</li>`)
          .join("")}</ul></section>`;
      }
